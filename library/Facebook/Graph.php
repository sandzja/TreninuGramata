...
if (!function_exists('curl_init')) {
	throw new Exception('Graph needs the CURL PHP extension.');
}
if (!function_exists('json_decode')) {
	throw new Exception('Graph needs the JSON PHP extension.');
}
 ...
 */
class Facebook_GraphApiException extends Exception {
...
/**
 * Provides access to the Graph Platform.
 *
 * @author Naitik Shah <naitik@facebook.com>
 */
class Facebook_Graph {
	 ...
	 * the current URL.
	 */
	protected static $DROP_QUERY_PARAMS = array ('session');

	/**
	 * Maps aliases to Graph domains.
	 */
	public static $DOMAIN_MAP = array ('api' => 'https://api.facebook.com/', 'api_read' => 'https://api-read.facebook.com/', 'graph' => 'https://graph.facebook.com/', 'www' => 'https://www.facebook.com/');
	...
	/**
	 * Initialize a Graph Application.
	 *
	 * The configuration:
	...
	public function getLogoutUrl($params = array()) {
		$session = $this->getSession();
		return $this->getUrl('www', 'logout.php', array_merge(array ('api_key' => $this->getAppId(), 'next' => $this->getCurrentUrl(), 'session_key' => $session['session_key']), $params));
	}
	...
	public function getLoginStatusUrl($params = array()) {
		return $this->getUrl('www', 'extern/login_status.php', array_merge(array ('api_key' => $this->getAppId(), 'no_session' => $this->getCurrentUrl(), 'no_user' => $this->getCurrentUrl(), 'ok_session' => $this->getCurrentUrl(), 'session_version' => 3), $params));
	}
	...
	protected function _restserver($params) {
		...
		if (is_array($result) && isset($result['error_code'])) {
			throw new Facebook_GraphApiException($result);
		}
		return $result;
	}
	...
	protected function _graph($path, $method = 'GET', $params = array()) {
		...
		if (is_array($result) && isset($result['error'])) {
			$e = new Facebook_GraphApiException($result);
	...
	protected function makeRequest($url, $params, $ch = null) {
		...
		$result = curl_exec($ch);
		if ($result === false) {
			$e = new Facebook_GraphApiException(array ('error_code' => curl_errno($ch), 'error' => array ('message' => curl_error($ch), 'type' => 'CurlException')));
			curl_close($ch);
			throw $e;
		}
		curl_close($ch);
	...
	protected function validateSessionObject($session) {
		...
		if (is_array($session) && isset($session['uid']) && isset($session['session_key']) && isset($session['secret']) && isset($session['access_token']) && isset($session['sig'])) {
			...
			$expected_sig = self::generateSignature($session_without_sig, $this->getApiSecret());
